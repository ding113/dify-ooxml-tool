...
class ExtractOoxmlTextTool(Tool):
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        try:
            ...
            logger.info(f"[ExtractOoxmlText] Storing extraction data to persistent storage - {len(text_segments)} segments, {len(file_data)} bytes original file")
    ...
    def _store_extraction_data(self, file_id: str, metadata: dict, text_segments: list, original_file: bytes):
        """Store extraction data in persistent storage."""
        logger.debug(f"[ExtractOoxmlText] Storing data for file_id: {file_id}, segments: {len(text_segments)}, file_size: {len(original_file)} bytes")
        try:
            # Optimize JSON serialization - do it once and reuse
            ...
            logger.debug(f"[ExtractOoxmlText] Storing metadata - Key: {metadata_key}, Size: {metadata_size} bytes")
            self.session.storage.set(metadata_key, metadata_bytes)
            
            # Store text segments
            texts_key = f"{file_id}_texts"
            texts_size = len(texts_bytes)
            logger.debug(f"[ExtractOoxmlText] Storing text segments - Key: {texts_key}, Size: {texts_size} bytes")
            self.session.storage.set(texts_key, texts_bytes)
            
            # Store original file
            file_key = f"{file_id}_original_file"
            original_size = len(original_file)
            logger.debug(f"[ExtractOoxmlText] Storing original file - Key: {file_key}, Size: {original_size} bytes")
            self.session.storage.set(file_key, original_file)
            
            total_stored_size = metadata_size + texts_size + original_size
            logger.info(f"[ExtractOoxmlText] All data stored successfully - Total size: {total_stored_size} bytes ({total_stored_size/1024/1024:.2f}MB)")
            
        except Exception as e:
            logger.error(f"[ExtractOoxmlText] Failed to store extraction data: {str(e)}")
            logger.debug(f"[ExtractOoxmlText] Storage error details - metadata_size: {metadata_size}, segments_count: {len(text_segments)}, file_size: {len(original_file)}")
